...
def _fake_venv_creation(
    ...
    *,
    ...
    state = {"pytest_installed": pytest_installed}
    ...
    def fake_run(command: list[str], **_kwargs) -> subprocess.CompletedProcess:
        ...
        if command[1:] == ["-c", "import pytest"]:
            return subprocess.CompletedProcess(command, 0 if state["pytest_installed"] else 1)
        if command[-1] == "pytest":
            state["pytest_installed"] = True
...
def test_prepares_and_reuses_a_project_virtual_environment(temp_path: Path, monkeypatch) -> None:
    ...
    assert calls[1][-2:] == ["-c", "import pytest"]
...
def test_installs_an_existing_requirements_file_and_tracks_its_fingerprint(
    ...
    assert calls[2][-2:] == ["-c", "import pytest"]
    assert calls[3][-1] == "pytest"
...
def test_repairs_an_environment_without_requirements_or_pytest(temp_path: Path, monkeypatch) -> None:
    project = Project("demo", ".", absolute_path=temp_path.as_posix())
    environment = PythonExecutionEnvironment.for_project(project)
    environment.python_executable.parent.mkdir(parents=True, exist_ok=True)
    environment.python_executable.touch()
    marker = environment.path / ".pymut4se-requirements"
    marker.write_text(environment._requirements_fingerprint(), encoding="utf-8")
    calls: list[list[str]] = []
    monkeypatch.setattr(
        "pymut4se.execution.environment.subprocess.run",
        _fake_venv_creation(environment, calls),
    ...
    assert calls[0][-2:] == ["-c", "import pytest"]
